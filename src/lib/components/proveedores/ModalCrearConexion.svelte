<script lang="ts">
  ...
  import type { ConnectionCreate } from '$lib/types/proveedores';

  const props = $props<{ open: boolean; onClose: () => void; onCreate?: (c: ConnectionCreate) => Promise<void> }>();

  const today = new Date().toISOString().slice(0, 10);
  let working = $state<ConnectionCreate>({
    bandwidth_down: 0,
    bandwidth_up: 0,
    ratio: '1:1',
    contract_date: today,
  ...
  async function create() {
    ...
    try {
      const payload: ConnectionCreate = {
        ...
        contract_date: working.contract_date,
  ...
  const bdValid = $derived(Number(working.bandwidth_down) > 0);
  const buValid = $derived(Number(working.bandwidth_up) > 0);
  const billingDayValid = $derived(Number(working.billing_day) >= 1 && Number(working.billing_day) <= 31);
  const contractDateValid = $derived(!!working.contract_date);
  const priceValid = $derived(Number(working.monthly_price) >= 0);
  const ratioTrim = $derived((working.ratio || '1:1').trim());
  const ifaceTrim = $derived((working.interface_name || '').trim());
  const pricePerMbPreview = $derived(bdValid ? Number(working.monthly_price) / Number(working.bandwidth_down) : 0);

  const canCreate = $derived(bdValid && buValid && billingDayValid && contractDateValid && priceValid);
</script>
    ...
    <Dialog.Positioner class="fixed inset-0 z-50 flex justify-center items-center p-4">
      <Dialog.Content class={"card bg-surface-100-900 w-[620px] max-w-[95vw] max-h-[80vh] overflow-auto p-4 space-y-4 shadow-xl rounded-2xl " + animation}>
        <header class="flex justify-between items-center">
          <Dialog.Title class="text-lg font-bold">Crear Conexión</Dialog.Title>
        ...
        <div class="flex justify-between items-center gap-3 pt-2">
          <div class="text-sm">
            ...
            {:else if createOk}
              <span class="text-emerald-400">Conexión creada</span>
            {:else if createErr}
