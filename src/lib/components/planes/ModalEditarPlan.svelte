<script lang="ts">
  ...
  import { formatMbps, pct, limitStringFromMbps, parsePlanRatioDivisor, ratioDivisorFromMaxAndGuaranteed, ratioStringFromDivisor } from '$lib/utils/plan-calculations';
  ...
  const props = $props<{ open: boolean; plan: Plan | null; onClose: () => void; onSave?: (plan: Plan) => Promise<Plan | void>; capacity?: CapacitySnapshot | null }>();

  let working = $state<Plan | null>(null);
  ...
  $effect(() => {
    working = props.plan ? { ...props.plan } : null;
    featuresList = [...(props.plan?.features || [])];
    const d = Number(props.plan?.download ?? 0);
    const div = parsePlanRatioDivisor(props.plan?.ratio);
    guaranteedTargetDown = d > 0 ? (d / Math.max(1, div)) : 0;
    newFeature = '';
  });
  ...
  const downloadValid = $derived(Number.isFinite(Number(working?.download)) && Number(working?.download) > 0);
  const uploadValid = $derived(Number.isFinite(Number(working?.upload)) && Number(working?.upload) > 0);
  const priceValid = $derived(Number.isFinite(Number(working?.price)) && Number(working?.price) >= 0);
  ...
  const computedDownloadLimit = $derived.by(() => limitStringFromMbps(Number(working?.download ?? 0)));
  const computedUploadLimit = $derived.by(() => limitStringFromMbps(Number(working?.upload ?? 0)));
  ...
  const ivaPreviewValid = $derived(priceValid && Number(working?.price ?? 0) > 0);
  ...
  const canSave = $derived.by(() => {
    return Boolean(working) && nameTrim.length > 0 && downloadValid && uploadValid && guaranteedTargetDownValid && priceValid;
  });
  ...
  async function save() {
    ...
    try {
      ...
      const priceToSend = (priceIncludesIva && ivaPreviewValid)
        ...
        : working.price;
      if (props.onSave) {
        const updated = await props.onSave({ ...working, price: priceToSend });
        if (updated) {
          working = { ...updated };
        ...
        {#if working}
          ...
          <div class="flex justify-between items-center gap-3 pt-2">
            <div class="text-sm">
              ...
              {:else if saveOk}
                <span class="text-emerald-400">Plan guardado</span>
              {:else if saveErr}
            ...
            <div class="flex gap-3">
              ...
              <button class="px-4 py-2 rounded-xl bg-gray-200 text-gray-900 text-sm font-semibold shadow-lg transition-colors disabled:opacity-60 disabled:cursor-not-allowed" disabled={isSaving || !canSaveWithCapacity} onclick={save}>{isSaving ? 'Guardando...' : 'Guardar'}</button>
            </div>
          </div>
        {:else}
