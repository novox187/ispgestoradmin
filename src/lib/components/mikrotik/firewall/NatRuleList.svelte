<script lang="ts">
	...
	import type { FirewallNatAction, FirewallNatRule } from '$lib/types/mikrotik-firewall';
	import { sortByPriority } from '$lib/utils/mikrotik-firewall-ordering';

	const props = $props<{
		rules: FirewallNatRule[];
		filterText?: string;
		filterAction?: FirewallNatAction | '';
	...
	const sorted = $derived(sortByPriority<FirewallNatRule>(props.rules));
	...
	function matchText(r: FirewallNatRule) {
		const parts: string[] = [];
	...
	function toText(r: FirewallNatRule) {
		const parts: string[] = [];
		if (r.toAddresses) parts.push(`to=${r.toAddresses}`);
		if (r.toPorts)     parts.push(`ports=${r.toPorts}`);
		return parts.length ? parts.join(' · ') : '—';
	}

	const actionColor: Record<string, string> = {
		masquerade: 'text-blue-300',
		'src-nat':  'text-emerald-300',
		'dst-nat':  'text-purple-300',
		redirect:   'text-amber-300'
	};
</script>
...
<div class="space-y-3">
	{#if props.rules.length === 0}
		<div class="border border-neutral-800 rounded-xl p-4 bg-neutral-900/30 text-sm text-gray-400">
			No hay reglas NAT en esta sección.
		</div>
	{:else}
		<!-- Barra de filtros -->
		<div class="flex flex-wrap items-center gap-2">
			<div class="relative flex-1 min-w-[180px] max-w-xs">
				...
				<input
					type="text"
					placeholder="Buscar en reglas NAT…"
					value={props.filterText ?? ''}
					oninput={(e) => props.onFilterTextChange?.((e.currentTarget as HTMLInputElement).value)}
					class="w-full pl-8 pr-3 py-1.5 rounded-lg border border-neutral-800 bg-neutral-900 text-xs text-gray-100 placeholder-gray-600 focus:outline-none"
				/>
			</div>
			<select
			...
			<table class="min-w-[980px] w-full text-sm">
				<thead class="bg-[#121214] text-gray-300">
					<tr>
						...
						<th class="text-left px-4 py-3 font-medium"><span class="inline-flex items-center gap-2"><GitBranch class="w-4 h-4 text-gray-400" /> Cadena</span></th>
						<th class="text-left px-4 py-3 font-medium"><span class="inline-flex items-center gap-2"><ArrowRight class="w-4 h-4 text-gray-400" /> Acción</span></th>
						<th class="text-left px-4 py-3 font-medium"><span class="inline-flex items-center gap-2"><Network class="w-4 h-4 text-gray-400" /> Proto</span></th>
						<th class="text-left px-4 py-3 font-medium"><span class="inline-flex items-center gap-2"><SlidersHorizontal class="w-4 h-4 text-gray-400" /> Match</span></th>
						<th class="text-left px-4 py-3 font-medium"><span class="inline-flex items-center gap-2"><ChevronRight class="w-4 h-4 text-gray-400" /> To</span></th>
						<th class="text-left px-4 py-3 font-medium"><span class="inline-flex items-center gap-2"><MessageSquare class="w-4 h-4 text-gray-400" /> Comentario</span></th>
						...
						>
							<td class="px-4 py-3">
								<div class="flex items-center gap-2">
									<button type="button" class="p-2 rounded-lg border border-neutral-800 bg-neutral-900/40 hover:bg-neutral-900/70 cursor-grab" onclick={(e) => e.preventDefault()}>
							...
							<td class="px-4 py-3">
								...
								>
									<Power class="w-3 h-3" /> {r.enabled ? 'ON' : 'OFF'}
								</button>
							</td>
							<td class="px-4 py-3"><span class="font-mono text-xs text-gray-300">{r.chain}</span></td>
							<td class="px-4 py-3"><span class="font-mono text-xs font-semibold {actionColor[r.action] ?? 'text-gray-300'}">{r.action}</span></td>
							<td class="px-4 py-3"><span class="font-mono text-xs text-gray-400">{r.protocol}</span></td>
							<td class="px-4 py-3 text-xs text-gray-300">{matchText(r)}</td>
							<td class="px-4 py-3 text-xs text-gray-400">{toText(r)}</td>
							<td class="px-4 py-3 text-xs text-gray-400 max-w-[140px] truncate" title={r.comment ?? ''}>{r.comment ?? '—'}</td>
							<td class="px-4 py-3">
								<div class="flex items-center justify-end gap-1.5">
									<button type="button" class="px-2.5 py-1.5 rounded-lg bg-neutral-800 hover:bg-neutral-700 text-gray-100 text-xs font-medium transition-colors inline-flex items-center gap-1.5" onclick={() => props.onEdit(r.id)}>
										<Pencil class="w-3.5 h-3.5" /> Editar
									</button>
									<button type="button" class="px-2.5 py-1.5 rounded-lg bg-red-600/20 hover:bg-red-600/30 text-red-300 text-xs font-medium transition-colors inline-flex items-center gap-1.5" onclick={() => props.onDelete(r.id)}>
