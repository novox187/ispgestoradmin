  ...
  async function submit() {
    ...
    try {
      ...
      if (res.ok) {
        dispatch('created');
        dispatch('close');
        form = { name: '', email: '', password: '', phone: '', role_id: undefined };
      } else {
        ...
        if (res.status === 422) {
            ...
            errorMsg = data.message || 'Error al crear usuario';
        }
      }
    } catch (e) {
...
<Dialog open={props.showAddUser} onOpenChange={() => dispatch('close')}>
  <Portal>
    ...
    <Dialog.Positioner class="fixed inset-0 z-50 flex justify-center items-center p-4">
      <Dialog.Content class="card bg-neutral-900 w-full max-w-lg p-6 space-y-4 shadow-xl rounded-xl border border-neutral-800 text-gray-100 {animation}">
        <header class="flex justify-between items-center">
          <Dialog.Title class="text-lg font-bold">Agregar Nuevo Usuario</Dialog.Title>
